...
# import shapefile
grid <- st_read("cedarcreek_cameragrid_latlong.shp") 
...
camera_metadata <- dat[c("site", "Easting", "Northing", "biome", "habitat_class", "plant_community", 
                         "LULC_info")]
dat <- subset(dat, select = -c(biome, habitat_class, plant_community, LULC_info))
camera_metadata <- camera_metadata[!duplicated(camera_metadata),] #97 sites 
names(camera_metadata)[4:7] <- c("Biome", "Habitat_Class", "Plant_Community", "LULC_Info")

# specify seasons for each month-year
seasons <- dat[c("Month", "season")]
seasons <- seasons[!duplicated(seasons),]
                       
# import camera operation spreadsheet
camera_operation <- read.csv("cedarcreek_expanded_searcheffort.csv")
